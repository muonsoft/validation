...
var regexConstraintTestCases = []ConstraintValidationTestCase{
	{
		name:            "IsRegexMatch passes on nil",
		isApplicableFor: specificValueTypes(stringType),
		options:         []validation.Option{it.IsRegexMatch("^[a-z]+$")},
		assert:          assertNoError,
	},
	{
		name:            "IsRegexMatch passes on empty value",
		isApplicableFor: specificValueTypes(stringType),
		options:         []validation.Option{it.IsRegexMatch("^[a-z]+$")},
		stringValue:     stringValue(""),
		assert:          assertNoError,
	...
	{
		name:            "IsRegexMatch violation ignored when condition false",
		isApplicableFor: specificValueTypes(stringType),
		options:         []validation.Option{it.IsRegexMatch("^[a-z]+$").When(false)},
		stringValue:     stringValue("1"),
		assert:          assertNoError,
	},
	{
		name:            "IsRegexMatch violation when condition true",
		isApplicableFor: specificValueTypes(stringType),
		options:         []validation.Option{it.IsRegexMatch("^[a-z]+$").When(true)},
		stringValue:     stringValue("1"),
		assert: assertHasOneViolation(
			code.Regex,
	...
	{
		name:            "IsRegexMatch violation with custom message",
		isApplicableFor: specificValueTypes(stringType),
		options: []validation.Option{
			it.IsRegexMatch("^[a-z]+$").Message(customMessage),
		},
		stringValue: stringValue("1"),
		assert:      assertHasOneViolation(code.Regex, customMessage, ""),
	},
	{
		name:            "IsRegexMatch passes on expected string",
		isApplicableFor: specificValueTypes(stringType),
		stringValue:     stringValue("a"),
		options:         []validation.Option{it.IsRegexMatch("^[a-z]+$")},
		assert:          assertNoError,
