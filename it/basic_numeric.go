...
// BlankNumberConstraint checks that a number value is blank: equal to nil or zero.
type BlankNumberConstraint[T validation.Numeric] struct {
	blank             T
	isIgnored         bool
	groups            []string
	code              string
...
// IsBlankNumber creates a BlankNumberConstraint for checking that value is empty.
func IsBlankNumber[T validation.Numeric]() BlankNumberConstraint[T] {
	return BlankNumberConstraint[T]{
		code:            code.Blank,
...
// When enables conditional validation of this constraint. If the expression evaluates to false,
// then the constraint will be ignored.
func (c BlankNumberConstraint[T]) When(condition bool) BlankNumberConstraint[T] {
	c.isIgnored = !condition
	return c
}

// WhenGroups enables conditional validation of the constraint by using the validation groups.
func (c BlankNumberConstraint[T]) WhenGroups(groups ...string) BlankNumberConstraint[T] {
	c.groups = groups
	return c
}

// Code overrides default code for produced violation.
func (c BlankNumberConstraint[T]) Code(code string) BlankNumberConstraint[T] {
	c.code = code
	return c
}
...
// Message sets the violation message template. You can set custom template parameters
// for injecting its values into the final message.
func (c BlankNumberConstraint[T]) Message(template string, parameters ...validation.TemplateParameter) BlankNumberConstraint[T] {
	c.messageTemplate = template
	c.messageParameters = parameters
	return c
}

func (c BlankNumberConstraint[T]) ValidateNumber(value *T, scope validation.Scope) error {
...
func (c BlankNumberConstraint[T]) newViolation(scope validation.Scope) validation.Violation {
	return scope.BuildViolation(c.code, c.messageTemplate).
		SetParameters(c.messageParameters...).
		CreateViolation()
}
