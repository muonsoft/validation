...
func TestValidator_Validate_WhenCustomFallbackLanguageAndUndefinedTranslationLanguage_ExpectFallbackLanguageUsed(t *testing.T) {
	validator, err := validation.NewValidator(
		validation.FallbackLanguage(language.Russian),
		validation.Translations(russian.Messages),
	)
	if err != nil {
		t.Fatal(err)
